...
def slice_len(df, name, name_latex_command):
    ...
    mean_slice_size_df = mean_slice_size_df.rename(
        columns={'dumb_dyn_slice_len': 'Executed statements', 'dyn_slice_len': 'Dynamic Slice (short circ. eval.)',
    ...
    idx = 0
    for key in ['Executed statements', 'Dynamic Slice (short circ. eval.)', 'Pruned Dynamic Slice']:
        for index, value in enumerate(mean_slice_size_df[key]):
            if value > 8 and idx==1:
                plt.text(value - 4.9, index - 0.33 + idx * 0.25, "{:.1f}".format(value), color='white', fontsize=9)
            elif value > 8:
                plt.text(value - 4.9, index - 0.33 + idx * 0.25, "{:.1f}".format(value), color='black', fontsize=9)
            else:
                plt.text(value + 1, index - 0.33 + idx * 0.25, "{:.1f}".format(value), fontsize=9)
        idx += 1

    save_and_show(name+'.png', name, slice_size_df.to_string())
...
def slice_reduction_boolops(df):
    ...
    # plt.gcf().suptitle('')
    # ax.set_title('')
    save_and_show('slice_size_reduction_boxplot.png', 'slice_size_reduction', df[cols].describe().to_string())
    ...
    # plt.gcf().suptitle('')
    # ax.set_title('')
    save_and_show('slice_size_reduction_boxplot_line.png', 'slice_size_reduction', df[cols].describe().to_string())
...
def latex_table(df):
    ...
    df_grouped = df_grouped.rename(
        columns={'dumb_dyn_slice_len': 'Executed statements', 'dyn_slice_len': 'Dynamic Slice',
                 'pruned_slice_len': 'Pruned Dynamic Slice', 'code_len': 'LOC', 'num_boolops': 'Boolean operands' })
...
def slice_length_passing_vs_failing(df, name):
    ...
    save_and_show(name + '.png', name, mean_slice_size_df.to_string())
